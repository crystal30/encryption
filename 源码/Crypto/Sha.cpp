...
void sha1_update( struct sha1_context *ctx, 
				  ...
				  uint32 length ) 
{ 
    uint32 left, fill; //left为填充数据时的左边起始位置
    ...
    ctx->total[0] += length <<  3; //长度左移3为能够计算出字符串长度位数
    ctx->total[1] += length >> 29; 
	
    ctx->total[0] &= 0xFFFFFFFF; 
    ctx->total[1] += ctx->total[0] < ( length << 3 ); 
	
    if( left && length >= fill ) //有数据且已填充完毕
    { 
...
{ 
    ...
    sha1_update( ctx, sha1_padding, padn ); //填充0x080...位数据信息
    sha1_update( ctx, msglen, 8 ); //填充长度信息
...
void Sha()
{
	unsigned char plaintext[] = "nanfaafjalflalavlala";
	...
	sha1_update(&sh, plaintext, len);

	sha1_finish(&sh,ciphertext);
