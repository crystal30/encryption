...
void Des()
{
	unsigned char destemp[8] = "Admin";
	unsigned char deskey[8];
	unsigned char ciphertext[8],plaintext[8];
	...
	des_decipher(ciphertext,plaintext,deskey);
	printf("%s\n",plaintext);
}

void Cbc_Des()
{
	unsigned char destemp[] = "Admin fjlaflka fnlaflkalkf^&*()_&*()&*(jflajflkalkfalkflka";
	unsigned char deskey[8];
	...
	cbc_encipher(destemp,ciphertext,deskey,len);
	cbc_decipher(ciphertext,plaintext,deskey,len);
	printf("%s\n",plaintext);
}